...
ALLOWLIST: dict[tuple[str, str], str] = {
    ...
    # Same shape as the other upload previews above: parses the zip and
    # reports what a restore would do, nothing more.
    ("POST", "/api/export/restore/preview"): "parses an upload and returns a preview; writes nothing",
    # Same family as POST /api/workspaces above: restore always creates a
    # brand-new workspace (never merges into an existing one — see
    # backup_restore_service), so there is no existing workspace to gate
    # against yet. The caller becomes its owner.
    ("POST", "/api/export/restore"): "creates the workspace there is no membership in yet",
    # Same shape as the other upload previews above: parses the B3 CSV/XLSX
    # and reports what an import would do, nothing more.
    ("POST", "/api/assets/import/b3-preview"): "parses an upload and returns a preview; writes nothing",
